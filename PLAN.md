...
- `../02-雄女電資班考題整理/videos/05-python-editor.md`
- `../02-雄女電資班考題整理/videos/06-selection-structure.md`
- `../02-雄女電資班考題整理/videos/07-loop-structure.md`
- `../02-雄女電資班考題整理/videos/08-builtins-and-custom-functions.md`
- `../02-雄女電資班考題整理/videos/09-objects-and-classes.md`
- `../02-雄女電資班考題整理/videos/10-modules-packages-programs.md`

第 1 到第 4 講為第一批；2026-07-10 已依校對完成狀態擴充到第 10 講，合計 32 節課程、145 題。

## 3.1 課程來源

上課功能同樣以 `../02-雄女電資班考題整理/videos/` 為主要來源。每一份影片整理筆記，會先轉成一個「課程小節」，再連到對應觀念與題目。

第一版課程來源先使用第 1 到第 4 講，和第一版題庫範圍一致。2026-07-10 已擴充到第 10 講；後續依學生錯題狀況再補混合題、錯題本與模擬考模式。
...
- 不能只寫定義摘要，必須用國三升高一學生能聽懂的語氣重寫。
- 每節課先說「這節到底要解決什麼問題」。
- 每節課至少放一個生活例子，把抽象觀念接到學生熟悉的情境。
- 每節課至少放一個可執行 Python 例子，讓學生能在網頁試跑。
- 每節課要列出常見誤解，特別是考題容易設陷阱的地方。
- 語句要像老師上課解釋，不要像字典條目或簡短筆記。
...
第一版原本只做第 1 到第 4 講，目標不是一次做完整網站，而是先讓學生開始練習，並驗證題目分級與弱點追蹤是否有效。第 5 到第 10 講校對完成後，已於 2026-07-10 擴充進網站。
...
| 第 5 講 | IDLE、VS Code、Python extension、註解、縮排 |
| 第 6 講 | `if`、`elif`、`else`、條件順序、巢狀條件、條件表達式、`pass` |
| 第 7 講 | `while`、`for`、`range()`、iterable、`break`、`continue`、巢狀迴圈、comprehension |
| 第 8 講 | 內建函式、自訂函式、參數、`return`、可變/不可變物件、作用範圍 |
| 第 9 講 | class、object、`__init__`、`self`、屬性、方法、繼承、覆寫、多重繼承 |
| 第 10 講 | 模組、套件、`import`、別名、`__name__ == "__main__"`、`dir()`、pip |
...
- 管理者登入。
- 課程清單與單一課程頁。
- 從第 1 到第 10 講建立第一批與後續擴充課程內容。
- 課程頁能執行 Python 範例。
- 記錄課程閱讀與完成狀態。
- 課程完成後進入對應練習。
- 從第 1 到第 10 講建立第一批與後續擴充題庫。
- 一次只顯示一題。
- 單選題、多選題、輸出預測題。
- 作答後立即顯示解析。
- 記錄作答結果。
...
- 依第 1 到第 10 講學生錯題狀況，補混合觀念題與高頻錯誤題。
- 補模擬考模式。
- 補錯題本。
- 補每日複習排程。
...
| 題庫需要持續增厚 | 第 1 到第 10 講已可練習，後續依學生錯題補混合題與弱點題 |
