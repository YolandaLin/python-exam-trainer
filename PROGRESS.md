...
- 狀態：已部署到 Render；第 1 到第 6 講課程 + 練習 MVP 可用。28 節課程已重寫成國三升高一可讀的上課講解版，並補到每節課至少 3 題小檢查/練習題，學生可先上課、完成小檢查，再進入對應練習題。
- 主要資料來源：`../02-雄女電資班考題整理/`。
...
| 課程資料 | `content/lessons.json` | 已重寫 | 已從 `../02-雄女電資班考題整理/videos/` 整理第 1 到第 6 講 28 節課程；每節補生活例子、可執行範例與常見誤解。 |
| 題目資料 | `content/questions.json` / `content/questions_1_4_extra.json` / `content/questions_5_6.json` | 已補密度 | 第 1 到第 6 講共 97 題；28 節課每節至少 3 題，第 5 講 7 題、第 6 講 12 題。 |
...
4. 等 `../02-雄女電資班考題整理/` 第 7 講以後整理穩定後，逐批補第 7 講以後課程與題庫。

## 已驗證

- `python scripts/check_questions.py`：通過，97 題、52 個觀念；會檢查第 1 到第 6 講每個來源小節至少 3 題，並實際執行輸出預測題比對答案。
- `python scripts/check_lessons.py`：通過，28 節課程、52 個觀念、84 個小檢查連結；檢查每節課需有生活例子、可執行範例、足夠解說長度與至少 3 題小檢查。
...
- 已完成：課程優先流程、28 節初學者版課程、97 題第 1 到第 6 講題庫、每節 3 題小檢查、Render 部署、PostgreSQL 支援、production 預設密碼阻擋、README 移除密碼欄。
- 本次新增：第 5 講「Python 程式語言編輯器」課程 + 7 題；第 6 講「選擇結構」課程 + 12 題。
- 下次接手先做：補管理者弱點檢視與課程完成率；之後再做密碼初始化/重設流程，或等第 7 講以後教材穩定後續補題庫。
